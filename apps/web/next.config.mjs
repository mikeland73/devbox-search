...
const nextConfig = {
  ...
  // Don't 308 `/v1/pkg/` to `/v1/pkg`. The Go service path.Clean'd every request,
  // so `/v1/pkg/`, `/v2/resolve/` and `/readyz/` were served by the same handler
  // as their slash-less form (a 400 for `/v1/pkg/`, not a redirect). With the
